...
|---|---|---|---|
| `langgraph/` | `ReducingSaver(InMemorySaver())` (stand-in for `langgraph-dynamodb-checkpoint`, `-cosmosdb`, `-firestore`, or any saver) | inside the checkpointer's `put` | `langgraph-memory` engine over a `BaseStore` |
...
# langgraph
uv add "agentstate-reducer[langgraph]" langgraph-memory langgraph-store-core langgraph langchain-core pytest
# crewai
