...
def build_engine(llm, embed) -> tuple[MemoryEngine, dict]:
    """MemoryEngine over an indexed InMemoryStore; returns (engine, retry counter)."""
    ...
    return engine, retries


def build_reducer(engine: MemoryEngine) -> tuple[MessageReducer, list]:
    """The bounded-window reducer with an on_prune hook = engine.remember + logging; returns (reducer, events)."""
    ...
    return reducer, prune_events


def log_turn(i: int, text: str, reply: str, window: int, events: list) -> int:
    """Print one turn's log block; returns the number of messages delivered to on_prune in this turn."""
    log(f"--- turn {i:02d} | user: {short(text)}")
    log(f"    agent: {short(reply, 110)}")
    log(f"    checkpoint holds {window} messages")
    delivered = 0
    for ev in events:
        delivered += len(ev["left"])
...
def run() -> dict:
    ...
    for i, text in enumerate(TURNS, 1):
        ...
        total_pruned += log_turn(i, text, out["messages"][-1].content, len(stored_now), prune_events[before:])
